...
public class NtfyAckSubscriber {
    ...
    public NtfyAckSubscriber(GatewayContext context, String profileName, String serverUrl, String topic) {
        ...
        this.ackTopic = topic;
        this.logger = LoggerEx.newBuilder().build(String.format("Ntfy.%s.AckSubscriber", profileName));
        ...
        executor.execute(() -> {
            while (!executor.isShutdown()) {
                ...
                logger.debugf("Sleeping for %s ms", reconnectTime);
                try {
    ...
    private void handleNonSuccessResponse(HttpURLConnection connection) {
        ...
        try {
            ...
            logger.errorf("Non-success response: %d -- %s", responseCode, responseMessage);
        } catch (IOException e) {
    ...
    private String decrypt(String encryptedData) throws NoSuchAlgorithmException, NoSuchPaddingException,
